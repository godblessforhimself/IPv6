...
'''
    ...
    2.随机地址生成 build prefix & predict database compress IP to int
        prefix file
        python APD.py -db APD_db1.db -generate
        建表predictTable
        输出预测目标
    3.结果统计
        一行是一个响应ip
        哪些前缀的响应率为100%
        python -statistics -i zmap.result -db database
        输出16个响应的前缀
    4.前缀响应分布比例 直方图 + 饼图
        python -db db -draw
'''
...
if __name__=='__main__':
    ...
    if args.generate:
        if args.input and args.database:
            ...
            connection.execute('create table {} (IP TEXT, prefix TEXT, responsive NUMERIC);'.format(predictTable))
            for prefix_ in connection.execute('select prefix from {} where count>=?'.format(prefixTable), (args.threshold,)):
                ...
                for target in targets:
                    connection.execute('insert into table {} values (?,?,0)'.format(predictTable),(target,prefix_[0],))
                    print(target)
            connection.commit()
            connection.close()
    ...
    if args.statistics:
        ...
        for line in open(args.input):
            ...
            prefix=connection.execute('select prefix from {} where IP=?'.format(predictTable),(raw_IP,))[0]
            connection.execute('update {} set responsive=1 where IP=?'.format(predictTable),(raw_IP,))
            connection.execute('update {} set response_count=response_count+1 where prefix=?'.format(predictTable),(prefix,))
    ...
    if args.draw:
        ...
        plt.bar(left=X, height=Y, width=0.8)
        plt.xlim(0,16)
